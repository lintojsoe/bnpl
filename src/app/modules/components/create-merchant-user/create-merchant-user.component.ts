...
@Component({
    ...
    styleUrls: ['./create-merchant-user.component.scss'],
    animations: FuseAnimations,
})
export class CreateMerchantUserComponent implements OnInit {
    id: any = '';
    breadcrumbs: Crumb[] = [];
    form: FormGroup;
    userDetails: any;
    ...
    async getDetails() {
        try {
            const userDetails = await this.userService
                .getUserDetailsMerchant(this.id)
                .toPromise();
            if (userDetails) {
    ...
    async formInit() {
        this.form = this.fb.group({
            ...
            account_id: [''],
            user: [this.userDetails ? this.userDetails.user : null],
        });
        let user: any = await this.utilitiesService.parseLocalStorageUser();
        console.log(user);
        if (this.id) {
        ...
        this.form.controls.account_id.setValue(user.account);
    }
            ...
            if (resp) {
                ...
                try {
                    ...
                    const addUser = await this.userService
                        .updateUserMerchant(form, this.id)
                        .toPromise();
                    if (addUser) {
